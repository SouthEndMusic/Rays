...
@testset "Scene_partition" begin
    ...
    n_cubes = 250
    scene = Rays.Scene()
    partition_nodes = scene.partition.partition_nodes
    ...
    function overlap(
        box_1::Rays.BoundingBox,
        box_2::Rays.BoundingBox;
    ...
    for node ∈ partition_nodes
        if node.dim_split != 0
            child_node_1 = partition_nodes[node.child_indices[1]]
            child_node_2 = partition_nodes[node.child_indices[2]]
...
@testset "TriangleShape_partition" begin
    ...
    @test isempty(triangle_shape.partition.partition_nodes)
    Rays.render!(scene)

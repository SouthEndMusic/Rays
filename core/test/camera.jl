...
@testset "set ray" begin
    ...
    ray = Rays.Ray(1)
    ray_loc = view(ray.loc, 1, :)
    ray_dir = view(ray.dir, 1, :)

    Rays.set_ray!(ray_loc, ray_dir, camera, (24, 68))
    @test ray isa Rays.Ray
    @test ray_loc ≈ Float32[0.6436098, 0.8657537, 0.15174258]
    @test norm(ray_dir) ≈ 1.0
    @test ray_dir ≈ Float32[0.2724516, -0.96194667, 0.020704657]
end

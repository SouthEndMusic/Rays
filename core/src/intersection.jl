...
function _intersect_ray!(
	...
	ray.loc .= view(intersection.rays_transformed[depth].loc, :)
	ray.dir .= view(intersection.rays_transformed[depth].dir, :)
	subshape_transform_intersect = nothing

	for subshape_transform in subshape_transforms
		inverse_transform!(ray, ray_camera, subshape_transform)
		closer_intersection_found = _intersect_ray!(intersection, shape)
	...
	if isnothing(subshape_transform_intersect)
		...
		if current_depth < depth
			...
			inverse_transform!(ray_transformed, ray, subshape_transform_intersect)
			closer_intersection_found = _intersect_ray!(
				intersection,
				fractal_shape,
				current_depth = current_depth + 1,
			)
			intersection.t[1] = transform_t(intersection, subshape_transform_intersect)
		else
			closer_intersection_found = true
		end
	end

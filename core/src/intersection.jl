...
function Base.convert(::Type{Intersection{F}}, intersection::Intersection) where {F}
	return Intersection{F}([getfield(intersection, fieldname) for fieldname in fieldnames(Intersection)]...)
end
...
function intersect!(
	...
	if !isnothing(subshape_intersect) && current_depth < depth
		...
		closer_intersection_found = intersect!(intersection, fractal_shape, current_depth = current_depth + 1)
		intersection.t[1] /= shrink_factor
	else
		closer_intersection_found = false
	end
...
function intersect!(
	...
	for i ∈ 1:shape.n_faces
		...
		t_int_candidate = intersect!(intersection, triangle_vertices; normal, t_int_prev = intersection.t[1])
		if t_int_candidate < intersection.t[1]
			closer_intersection_found = true
			intersection.t[1] = t_int_candidate
